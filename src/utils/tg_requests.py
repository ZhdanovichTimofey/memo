...
from config.config import POSSIBLE_EXTENTIONS, POSSIBLE_TYPES, TELEGRAM_TOKEN
from src.utils.exceptions import TooBigFile

__all__ = ["get_file", "send_message"]

req = {"POST": requests.post, "GET": requests.get}
...
def get_file(file_id):
    resp = _make_request(method="getFile", param_name="file_id", param_val=file_id)
...
def send_message(chat_id, text):
    _make_request(method="SendMessage", data={"chat_id": chat_id, "text": text})
